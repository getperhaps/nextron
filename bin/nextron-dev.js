...
(async () => {
  ...
  const startMainProcess = () => {
    ...
    mainProcess = execa('electron', ['.', `${rendererPort}`, `${electronOptions}`], _objectSpread({
      detached: true
    }, execaOptions));
    mainProcess.unref();
  };
  const startRendererProcess = () => {
    ...
    info(`Run renderer process: cd ${rendererDirectory} && next -p ${rendererPort}`);
    const child = execa('sh', ['-c', `cd ${rendererDirectory} && yarn next -p ${rendererPort}`], execaOptions);
    child.on('close', () => {
      process.exit(0);
    });
    return child;
